...
std::vector<std::string> search_advanced(const std::string& hint, const std::string& starting_path) {
        ...
        for (const auto& entry : fs::recursive_directory_iterator(starting_path, fs::directory_options::skip_permission_denied)) {
            try {
                if (entry.is_regular_file()) {
                    std::string entry_name = entry.path().filename().string(); // Get file/directory name
                    if (containsHint(entry_name, hint)) {
    ...
    // Sort result by length of filenames
...
static PyMethodDef SearchMethods[] = {
    {"search_advanced", py_search_advanced, METH_VARARGS, "Search files with hint and sort by length"},
    {NULL, NULL, 0, NULL}
};
